...
type Session interface {
	...
	WrapKey(mechanism pkcs11.Mechanism, wrappingKey, key Object) ([]byte, error)
	// UnwrapKey unwraps (decrypts) a wrapped key, creating a new key object.
	UnwrapKey(mechanism pkcs11.Mechanism, unwrappingKey Object, wrappedKey []byte, template []*pkcs11.Attribute) (Object, error)
	// DeriveKey derives a key from a base key, creating a new key object.
	DeriveKey(mechanism pkcs11.Mechanism, baseKey Object, template []*pkcs11.Attribute) (Object, error)
	// Digest digests message in a single part.
	Digest(mechanism pkcs11.Mechanism, message []byte) ([]byte, error)
	...
	SetPIN(oldPin, newPin []byte) error
...
func (s *sessionImpl) SetPIN(oldPin, newPin []byte) error {
	s.Lock()
	defer s.Unlock()
...
func (s *sessionImpl) GenerateKey(mechanism pkcs11.Mechanism, template []*pkcs11.Attribute) (Object, error) {
	s.Lock()
	defer s.Unlock()
	handle, err := s.ctx.GenerateKey(s.handle, []*pkcs11.Mechanism{&mechanism}, template)
	if err != nil {
		return Object{}, err
	}
	return Object{
...
// WrapKey wraps (i.e., encrypts) a key.
func (s *sessionImpl) WrapKey(mechanism pkcs11.Mechanism, wrappingKey, key Object) ([]byte, error) {
	s.Lock()
	defer s.Unlock()
	wrappedKey, err := s.ctx.WrapKey(s.handle, []*pkcs11.Mechanism{&mechanism}, wrappingKey.objectHandle, key.objectHandle)
	if err != nil {
		return nil, err
	}
...
// UnwrapKey unwraps (decrypts) a wrapped key, creating a new key object.
func (s *sessionImpl) UnwrapKey(mechanism pkcs11.Mechanism, unwrappingKey Object, wrappedKey []byte, template []*pkcs11.Attribute) (Object, error) {
	s.Lock()
	defer s.Unlock()
	unwrappedKey, err := s.ctx.UnwrapKey(s.handle, []*pkcs11.Mechanism{&mechanism}, unwrappingKey.objectHandle, wrappedKey, template)
	if err != nil {
		return Object{}, err
	}
	return Object{
...
// DeriveKey derives a key from a base key, creating a new key object.
func (s *sessionImpl) DeriveKey(mechanism pkcs11.Mechanism, baseKey Object, template []*pkcs11.Attribute) (Object, error) {
	s.Lock()
	defer s.Unlock()
	derivedKey, err := s.ctx.DeriveKey(s.handle, []*pkcs11.Mechanism{&mechanism}, baseKey.objectHandle, template)
	if err != nil {
		return Object{}, err
	}
	return Object{
		session:      s,
		objectHandle: derivedKey,
	}, nil
}

// Digest digests message in a single part.
func (s *sessionImpl) Digest(mechanism pkcs11.Mechanism, message []byte) ([]byte, error) {
	s.Lock()
	defer s.Unlock()

	err := s.ctx.DigestInit(s.handle, []*pkcs11.Mechanism{&mechanism})
	if err != nil {
		return nil, fmt.Errorf("digest init: %w", err)
	}

	hash, err := s.ctx.Digest(s.handle, message)
	if err != nil {
...
// Handle returns the handle of an object.
func (s *sessionImpl) Handle() pkcs11.SessionHandle {
	return s.handle
}
