...
type Session interface {
	...
	GenerateKey(mechanism pkcs11.Mechanism, keyAttributes []*pkcs11.Attribute) (*Object, error)
...
// GenerateKey generates a secret key, creating a new key object.
func (s *sessionImpl) GenerateKey(mechanism pkcs11.Mechanism, keyAttributes []*pkcs11.Attribute) (*Object, error) {
	s.Lock()
	defer s.Unlock()
	handle, err := s.ctx.GenerateKey(s.handle, []*pkcs11.Mechanism{&mechanism}, keyAttributes)
	if err != nil {
		return nil, err
	}
	return &Object{
...
func (s *sessionImpl) WrapKey(mechanism pkcs11.Mechanism, wrappingKey, key pkcs11.ObjectHandle) ([]byte, error) {
	s.Lock()
	defer s.Unlock()
	wrappedKey, err := s.ctx.WrapKey(s.handle, []*pkcs11.Mechanism{&mechanism}, wrappingKey, key)
	if err != nil {
		return nil, err
	}
